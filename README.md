...
3. **最终取 0.7**：误配为 0 的前提下加权代价最低（0.5 有约束时误配仍有 2 处，不划算）。

代价是诚实的：11 个正样本中 7 个漏配，其中约 18%（2 个）是"修改 + 小幅移动"的组合型修改——位置约束从原理上就会把它们拦成删+增，因为"删旧插新"与"改完再挪"在信息论上不可分。如实报告，比假装识别更可信（见已知边界）。

![amb 样本前后对比：修改+移动类样本被位置约束按设计降级为删+增](docs/screenshots/amb-before.png)
![有位置约束后：误配清零，降级如实呈现](docs/screenshots/amb-after.png)
...
> 一个坦白列出自己不支持什么的 README，比一个只说自己支持什么的 README 可信得多——因为它说明作者真的探过底。
