...
5. **特征解耦：位置约束解放相似度阈值**
   本系统的段落相似度阈值最终定为 0.5。在前期扫参实验中，我发现纯文本的字符级相似度根本无法区分“近距离的同义改写”与“远距离的相似模板句”，两者取值区间严重重叠。
   面对高昂的误配代价，我没有选择盲目拔高相似度阈值，而是引入了基于 LCS `unchanged` 锚点的“局部坐标系位置约束”。实验证明，位置约束（`POSITION_WINDOW = 2`）成功将跨段落模板句的误配率物理清零。这种特征解耦的设计，使得系统不仅能坚守“零误配”的红线，还能安全地将相似度阈值下探至 0.5，大幅挽回了同义改写的漏配损失。