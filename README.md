...
5. **相似度阈值的取舍与段内 Diff**
   为了将一删一增合并为修改，在 refine.py 中使用了 difflib.SequenceMatcher。经过样本实测，将 SIM_THRESHOLD 设为 0.6 能够平衡微调修改和重写全新段落之间的界限。MVP 阶段的段内 diff 同样暂时基于 difflib 提供词与字符级差异比对，在 v2 版本中，计划将这部分替换为自研的精细化词级 LCS 算法。