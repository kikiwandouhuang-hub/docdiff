...
段内词级 LCS 是 O(mn) 时间与空间。长段落会先走句级两级细化（段落 → 句子 → 词），全文无句界时退回 difflib 兜底——diff 工具卡住比 diff 得不够漂亮严重得多。以下数字来自 `experiments/gen_big_docx.py` 生成的合成文档（macOS / Python 3.13）：

| 文档规模 | 变更比例 | 耗时 | 峰值内存 |
|---|---|---|---|
| 100 段 × 50 字 | 5% | 0.03s | 16 MB |
| 500 段 × 50 字 | 5% | 0.04s | 19 MB |
| 2000 段 × 50 字 | 5% | 0.38s | 53 MB |
| 500 段 × 50 字 | 50% | 0.18s | 20 MB |
| 20 段 × 2000 字，1 处修改 | — | 0.02s | 17 MB |

最后一行是段内细化的句级路径：2000 字段落若直接跑词级 LCS 需要 400 万格的 dp 表，先按句子对齐后只需 200×200 加若干句内小表。50% 变更那行暴露了掐头去尾失效时的真实成本（0.04s → 0.18s），印证了"最坏复杂度不变，降低的是真实输入下的计算量"。
