...
5. **相似度配对的局限性与位置约束策略**
   在处理段落位移时，本系统摒弃了维护全局累积偏移量的脆弱做法。而是利用 LCS 算法自带的 `unchanged` 节点作为天然锚点，建立局部坐标系，直接换算出预期的对应位置，大幅提升了算法针对远距离相似模板句的抗误配能力。
   同时，必须明确本系统的已知边界：当段落**同时发生文本修改与跨段位移**时，系统会将其保守地报告为独立的“删除”与“新增”。因为在缺乏唯一段落 ID 的纯文本比对中，此类操作与“删除旧段落并在此处新增无关段落”在信息论层面完全不可区分。工具选择诚实的保守报告，拒绝用猜测污染变更的置信度。